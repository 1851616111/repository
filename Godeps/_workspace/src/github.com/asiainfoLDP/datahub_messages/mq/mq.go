...
func NewMQ(brokerList []string /*, zookeepers string*/ /*, c *Config*/) (*KafukaMQ, error) {
	var err error
...
func (mq *KafukaMQ) runComsumer(consumer *KafukaComsumer) {
	for {
		...
		case m := <-consumer.partitionConsumer.Messages():
			consumer.massageListener.OnMessage(m.Key, m.Value, m.Topic, m.Partition, m.Offset)
	...
	go func() {
		for {
			...
			case <-closed:
				goto END
			case m := <-msgs:
				consumer.ConsumeMessage(m.Key, m.Value, m.Topic, m.Partition, m.Offset)
			case e := <-errs:
				consumer.OnError(e)
			}
		}
