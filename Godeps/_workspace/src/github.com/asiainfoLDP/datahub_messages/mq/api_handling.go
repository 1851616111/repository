...
func (listener *ApiRequestListener) OnMessage(key, value []byte, topic string, partition int32, offset int64) {
	//log.Debugf("(%d) Message consuming key: %s, value %s", offset, string(key), string(value))
	if len(key) ==0 && len(value) == 0 {
		return // this is a message to create a topic, so it will be ignored
	}
	...
	if mq_request.ResponseTopic == mqp_VOID_MESSAGE_TOPIC {
		return // async requests don't need response
	}
